 ...
 */
export function formatSimulation(result: SimulationResult, startDate: Date): string {
  const lines: string[] = [];
 ...
 */
export function formatForecast(forecast: ForecastEntry[], showAll = false): string {
  ...
  if (showAll) {
    ...
    while (i < forecast.length) {
      const startEntry = forecast[i];
      if (startEntry === undefined) break;
      let end = startEntry;
      let j = i + 1;

      // Find consecutive days with same usedDays
      while (j < forecast.length) {
        const nextEntry = forecast[j];
        if (nextEntry === undefined || nextEntry.usedDays !== startEntry.usedDays) break;
        end = nextEntry;
        j++;
      }

      const bar = createProgressBar(startEntry.usedDays, ABSENCE_LIMIT, 20);

      if (i === j - 1) {
        // Single day
        lines.push(
          `${formatDate(startEntry.date)}: ${startEntry.usedDays.toString().padStart(3)} used, ${startEntry.availableDays.toString().padStart(3)} avail ${bar}`,
        );
      } else {
        // Range of days
        lines.push(
          `${formatDate(startEntry.date)} to ${formatDate(end.date)}: ${startEntry.usedDays.toString().padStart(3)} used, ${startEntry.availableDays.toString().padStart(3)} avail ${bar}`,
        );
      }
